        ...
        for (a = 0; a < d.length; a++) {
            obj = document.getElementById(d[a]); if (obj) {
                ...
                if (obj.shape.toLowerCase() == 'rect') {
                    ...
                    x = new Array(); y = new Array(); i = 0; for (j = 0; j < v.length; j += 2) { x[i] = parseInt(v[j]); y[i] = parseInt(v[j + 1]); i++; } jg[z].fillPolygon(x, y);
                } jg[z].paint();
            }
        }
    }
                ...
                if (mapid[0].areas[j].shape.match(/(rect|poly|circle)/i)) {
                    if (window.opera || mapid[0].areas[j].coords != '') {
                        ...
                        if (isVM || isIE) {
                            ...
                            mapid[0].areas[j].onclick = new Function('setAreaSelected(' + canvas.id + '' + ')');
