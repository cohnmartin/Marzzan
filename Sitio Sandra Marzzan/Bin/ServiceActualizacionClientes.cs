...
public class ServiceActualizacionClientes : System.Web.Services.WebService
{
        ...
        foreach (CommonMarzzan.Cliente item in clientes)
        {
            try
            {
                CommonMarzzan.Cliente clienteServive = (from C in db.Clientes
                                                        ...
                                                        select C).Single<CommonMarzzan.Cliente>();
    ...
    public void ActualizacionPresentacionesPromosyOtros(List<Presentacion> presentaciones)
    {
        ...
        foreach (Presentacion item in presentaciones)
        {
            Presentacion pre = (from C in dbLocal.Presentacions
                                where C.IdPresentacion == item.IdPresentacion
                                select C).First<Presentacion>();
    ...
    public void ActualizacionPPresentaciones(List<Presentacion> presentaciones)
    {
        ...
        /// busco y elimino aquellos productos que no tiene hijos ya sea productos en si
        /// o presentaciones.
        /// 
        try
        {
            #region Nivel 3
            List<Producto> ProductosaEliminar = (from P in dbLocal.Productos
                                                 where P.Nivel == 3 && P.ColPresentaciones.Count == 0
                                                 && P.Tipo == 'A'
                                                 select P).ToList<Producto>();


            List<long> ids = (from P in ProductosaEliminar
                              select P.IdProducto).ToList<long>();


            var ComposicionesEliminar = from C in dbLocal.Composicions
                                        where ids.Contains(C.objProductoHijo.IdProducto)
                                        select C;


            dbLocal.Composicions.DeleteAllOnSubmit<Composicion>(ComposicionesEliminar);

            dbLocal.Productos.DeleteAllOnSubmit<Producto>(ProductosaEliminar);
        ...
        try
        {
            #region Nivel 2
            List<Producto> ProductosaEliminar = (from P in dbLocal.Productos
                                  where P.Nivel == 2 && P.ColHijos.Count == 0
                                  && P.Tipo == 'A'
                                  select P).ToList<Producto>(); ;
       ...
       try{
            #region Nivel 1
           List<Producto> ProductosaEliminar = (from P in dbLocal.Productos
                                  where P.Nivel == 1 && P.ColHijos.Count == 0
                                  && P.Tipo == 'A'
                                  select P).ToList<Producto>(); ;
                ...
                if (ProductosExistentes == 0)
                {
                    ...
                    for (int NivelaCrear = nivel; NivelaCrear < 4; NivelaCrear++)
                    {
                        ...
                        newprod.objPadre = (from P in productosLocales
                                            ...
                                            select P).Distinct<Producto>().First<Producto>();
                        newprod.Image = "";
